...
from .common import args_on_debug_logger, main_func_log, setup_logger
...
def convert_coverage_data(dat_dir, out_dir, dat_pattern="coverage*.dat"):
    ...
    files = Path.glob(dat_dir, f"**/{dat_pattern}")

    if not files:
        logger.error("No coverage data files were found.")
        logger.error(f"Searched directory: {dat_dir.absolute}")
        logger.error(f"{__name__} ended with errors")
        raise Exception("No coverage data files were found.")
