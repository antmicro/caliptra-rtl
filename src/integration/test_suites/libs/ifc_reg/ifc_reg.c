...
const ifc_register_info_t REG_GROUP_KNOWN_VALUES_ARRAY [] = {
    { CLP_SOC_IFC_REG_CPTRA_HW_REV_ID, REG_NOT_STICKY, true },
...
const ifc_register_info_t REG_GROUP_CAPABILITIES_ARRAY [] = {
    ...
    { CLP_SOC_IFC_REG_CPTRA_FW_REV_ID_0, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_FW_REV_ID_1, REG_NOT_STICKY, false },
    { 0, REG_NOT_STICKY, false }  // End marker
};
...
const ifc_register_info_t REG_GROUP_DBG_MANUF_SERVICE_ARRAY [] = {
    { CLP_SOC_IFC_REG_SS_DBG_MANUF_SERVICE_REG_REQ, REG_EXT_LOCK, false },
    { CLP_SOC_IFC_REG_SS_DBG_MANUF_SERVICE_REG_RSP, REG_EXT_LOCK, false },
...
const ifc_register_info_t REG_GROUP_TRNG_RO_ARRAY [] = {
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_0, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_1, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_2, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_3, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_4, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_5, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_6, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_7, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_8, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_9, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_10, REG_NOT_STICKY, false },
    { CLP_SOC_IFC_REG_CPTRA_TRNG_DATA_11, REG_NOT_STICKY, false },
    { 0, REG_NOT_STICKY, false }  // End marker
};
 ...
 */
 int set_reg_exp_data(ifc_reg_exp_dict_t *dict, uint32_t address, uint32_t value, uint32_t mask, bool reg_write, ifc_register_group_t group_index_arg, bool soc_access) {
    ...
    VPRINTF(MEDIUM, "Register Name = %s[%d]\n", get_group_name(group_index), reg_index);
                ...
                if (reset_type == COLD_RESET) {
                    if (reg->has_init_value == false && ro_reg == false) {
                        ...
                        if (reg->address == CLP_SOC_IFC_REG_INTERNAL_RV_MTIME_H || reg->address == CLP_SOC_IFC_REG_INTERNAL_RV_MTIME_L && read_data >= exp_data) {
                            VPRINTF(MEDIUM, "  Expect reg increment: %s[%d] (0x%08x): Read 0x%08x, Expected 0x%08x\n", get_group_name(group), i, reg->address, read_data, exp_data);
                        } else if (read_data == exp_data) {
                            VPRINTF(MEDIUM,"  Match: %s[%d] (0x%08x): Read 0x%08x, Expected 0x%08x\n", get_group_name(group), i, reg->address, read_data, exp_data);
                        ...
                        if (get_reg_exp_data(&g_expected_data_dict, reg->address, &exp_data) == 0) {
                            ...
                            VPRINTF(LOW, "  ! %s[%d] (0x%08x): Read 0x%08x, No expected data in dictionary\n",
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (reg) {
            ...
            if (!is_register_excluded(reg->address)) {
                ...
                VPRINTF(MEDIUM, "  Reading 0x%08x from %s[%d] (0x%08x)\n", read_data, get_group_name(group), i, reg->address);
